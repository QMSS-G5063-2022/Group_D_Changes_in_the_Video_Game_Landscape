...
dly.f$`Initial Release Date` <-
  as.Date(dly.f$`Initial Release Date`, tz = "")
dly.f$`Final Release Date` <-
  as.Date(dly.f$`Final Release Date`, tz = "")
country <- read_xlsx("data/game delay.xlsx", sheet = 2)
country2 <- read_xlsx("data/game delay.xlsx", sheet = 3)
...
gamesall <- read_xlsx("data/gamesall_2.xlsx")
gmls <- read_xlsx("data/gamesall.f.xlsx")
...
sales$Date <-
  as.Date(with(sales, paste(Year, Month, Day, sep = "-")), "%Y-%m-%d")
stock <- read_xlsx("data/stock.xlsx")
stock$Date <-
  as.Date(with(stock, paste(timestamp, sep = "-")), "%Y-%m-%d")
...
dly_plt <-
  ...
  ggtitle("Delayed Games Over Time 2019-2022") +
  ylab(NULL) +
  xlab("Date") +
  geom_vline(xintercept = as.numeric(as.Date("2020-03-27")), linetype = 4) +
...
myplot2 <- ggplot(dly.f,
  ...
  ggtitle("Number of Days Game Delayed 2019-2022") +
  ylab("") +
...
salesplot <-
  ...
  geom_point() +
  geom_line() +
  geom_text(aes(x = as.Date("2019-11-30", "%Y-%m-%d"),
                y = 1920, color = "NS"), label = "NS Peak") +
  ylab("Sold (# of units per thousand) ") +
  ggtitle("Console Sales from 2019-2022") +
  theme_tufte() 

salesplot2 <- salesplot + scale_color_manual( values = c("lightgreen","lightsalmon","cyan3","lightslateblue","firebrick2","plum1","yellow1"))



salesplot3 <- ggplotly(salesplot2, tooltip = c("Hardware", "Sold")) 
